<div class="mainCont">
<form action="https://money.yandex.ru/quickpay/confirm.xml" target="_top" method="POST">
<input type="hidden" value="<?php echo MG::getOption('yandexPurse')?>" name="receiver">
<input type="hidden" value="Оплата заказа № <?php echo $data['id']?> " name="label">
<input type="hidden" value="Оплата товара/услуги" name="FormComment">
<input type="hidden" value="Оплата товара/услуги" name="short-dest">
<input type="hidden" value="false" name="writable-targets">
<input type="hidden" value="false" name="writable-sum">
<input type="hidden" value="false" name="comment-needed">
<input type="hidden" value="shop" name="quickpay-form">
<input type="hidden" value="Оплата заказа № <?php echo $data['id']?>" name="targets">
<input type="hidden" value="<?php echo $data['summ']?>" name="sum">
<input type="submit" class="btn" value="Оплатить" name="submit-button">

</form>
<p>
 <br/>
 <em>
 Вы сможете сделать оплату позже, перейдя по ссылке из высланного вам письма.
 <br/>
 Или перечислить деньги на наш <b><span style="color:red" >Я</span>ндекс</b> кошелек <b><?php echo MG::getOption('yandexPurse')?></b> указав номер заказа.
 <br/>
 А также вы сможете оплатить свой заказ спомощью нашей <a href="#">формы оплаты заказов</a>
 </em>
 </p>
<br/>
<img alt="" src="data:image/png;base64," />
</div>