  ...
  function getPageList($page = 1, $step = 5, $mgadmin = false){
    ...
    // перебирает все страницы и формирует ссылки на них.
      ...
      for($page = 0; $page < $count; $page++){
        ...
        if(!$mgadmin){
          $pages .= '<a class = "'.$class.'" href = "'.SITE.'/'.$urlPage.'?p='.($page + 1).'">'.($page + 1).'</a>'.$br1;
        } else{
          $pages .= '<a rel="pagination" page = "'.($page + 1).'" class = "'.$class.'" href = "#">'.($page + 1).'</a>'.$br1;
        }
      }
       ...
       if('catalog' == $this->currentCategory['url']){
         ...
         $sql = '
           ...
           LEFT JOIN category c
             ON c.id = p.cat_id
           WHERE c.id = %d '.$filter.'
           ORDER BY image_url DESC LIMIT %d , %d
         ';
